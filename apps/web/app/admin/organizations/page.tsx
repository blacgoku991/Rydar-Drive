import { ORG_STATUS_META, formatDate, type OrgStatus } from "@rydar/shared";
...
export default async function OrganizationsPage() {
  ...
  const [{ data: orgs }, { data: plans }] = await Promise.all([
    session.supabase.from("organizations").select("id, name, slug, status, city, email, created_at, plan:plans(name)").order("created_at", { ascending: false }),
    ...
    <>
      ...
      <PageBody>
        <Card className="overflow-hidden">
          <Table>
            <THead>
              <tr><TH>Rattacheur</TH><TH>Identifiant</TH><TH>Offre</TH><TH>Contact</TH><TH>Créé le</TH><TH>Statut</TH></tr>
            </THead>
            <tbody>
              {(orgs ?? []).map((o: any) => (
