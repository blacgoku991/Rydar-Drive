"use server";
// Inscription publique d'un chauffeur par le lien d'une centrale (/rejoindre/{code}), sans connexion.
...
export async function applyToCentrale(code: string, input: z.input<typeof joinApplicationSchema>): Promise<JoinResult> {
  ...
  if (!CODE_RE.test(joinCode)) return { ok: false, error: LINK_INACTIVE };
