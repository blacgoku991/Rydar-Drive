// Style de carte « Rydar Night / Day » — MapLibre, schéma OpenMapTiles.
// Partagé par le dashboard web et l'app chauffeur (version web).
// Production : tuiles OpenFreeMap (gratuites, sans clé) ; dev : tuiles locales.

/** StyleSpecification MapLibre (typage léger pour ne pas dépendre de maplibre-gl ici). */
export type MapStyle = { version: 8; name: string; glyphs: string; sources: Record<string, unknown>; layers: Record<string, unknown>[] };

export const DEFAULT_MAP_TILES = "https://tiles.openfreemap.org/planet";
export const DEFAULT_MAP_GLYPHS = "https://tiles.openfreemap.org/fonts/{fontstack}/{range}.pbf";
...
export function rydarMapStyle(theme: "night" | "day" = "night", opts: { tiles?: string; glyphs?: string } = {}): MapStyle {
  const c = theme === "night" ? NIGHT : DAY;
  const tiles = opts.tiles ?? DEFAULT_MAP_TILES;
  ...
  return {
    ...
    glyphs: opts.glyphs ?? DEFAULT_MAP_GLYPHS,
    sources: {
